resource "aws_route53_record" "expense" {
  ...
  zone_id = var.zone_id
  name    = var.instances[count.index] == "frontend" && var.environment == "prod" ? var.domain_name : "${var.instances[count.index]}-${var.environment}.${var.domain_name}"#interpolatio to get mysql-dev.ardev.space
  type    = "A"
  ttl     = 1
